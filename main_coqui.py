...
def generate_response(text):
    response = llm_model(text, max_tokens=300, top_p=0.95, top_k=60)
...
def text_to_speech(text, response_path):
    try:
        # 使用Coqui TTS生成语音
        tts_model.tts_to_file(text=text, file_path=response_path)
        
        # 播放音频
        fs, data = wavfile.read(response_path)
        sd.play(data, fs)
        sd.wait()

        print(f"Response played and saved to {response_path}.")
